...
# --- CONFIGURATION ---  PUT FILE NAMES HERE
...
def main():
    ...
    for k in range(num_samples_vis):
        ...
        raw_lim[k] = lim_base * (1.0 - 0.3 * max_int)
        
        t_curr = times_vis[k]
        # Raised base elevation to 55 degrees to look downward onto the "face" of the diagonal 3D attractor
        raw_elev[k] = 55.0 + 12.0 * np.sin(t_curr * 0.15) + 3.0 * np.cos(t_curr * 0.4) * int_v_list[0][k]
        
        f_approx = k * (FPS / VIS_SR)
        raw_azim[k] = f_approx * 0.5 + 25.0 * np.sin(t_curr * 0.15) * int_v_list[1][k]
        
    # Savitzky-Golay filter to smooth all high-frequency camera twitching (large integration window)
    filter_win = 2101  # ~7.0 second integration window at 300Hz (creates massive inertia)
    ...
    track_cmaps = []
    # Direct fixed assignments for priority heads: 1 -> Bass (Viridis), 2 -> Vocals (Spring)
    for i in range(X_HEADS):
        if i == 0:
    ...
    # Declare Master Combined Knot Collections
